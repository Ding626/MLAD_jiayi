...
int main(int argc, char** argv) {
  ...
  bool use_sim_state;
  if (!nh.getParam("use_sim_state", use_sim_state)) {
    ROS_ERROR("Failed to get param %d", use_sim_state);
    assert(false);
  }
  ...
  semantic_map_manager::EgoSemanticMapManager semantic_map_manager(
      ego_id, agent_config_path, true);
  semantic_map_manager::EgoRosAdapter smm_ros_adapter(nh, &semantic_map_manager, true, ego_id);
  ...
  p_ssc_server_ =
      new planning::SscPlannerServer(nh, ssc_planner_work_rate, ego_id, use_sim_state);
