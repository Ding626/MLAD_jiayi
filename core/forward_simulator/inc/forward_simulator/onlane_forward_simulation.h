...
namespace planning
{
  ...
  public:
    ...
    struct Param
    {
      ...
      decimal_t steer_control_max_lookahead_dist = 50;
        ...
        const decimal_t &dt, const Param &param, State *desired_state)
    {
      ...
      {
        // ~ Without leading vehicle
        // NOTE for MLAD: 這裏不應該用target_pos，因爲兩條道路起點不同，對同一個點，其frenet坐標系下的s不一樣，會出大問題
        // NOTE for MLAD: 即對一條沒有father路徑，而目標路徑有father路徑，其lane的起點不同，對應當前state的frenet坐標系的s也不一樣，有father的路徑的s會大很多，使得算法判定velocity應該減少爲0
