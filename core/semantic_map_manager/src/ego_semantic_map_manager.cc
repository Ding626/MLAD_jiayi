...
EgoSemanticMapManager::EgoSemanticMapManager(const int &id,
...
EgoSemanticMapManager::EgoSemanticMapManager(const int &id,
                                             const std::string &agent_config_path,
                                             const bool &is_auto_forward)
    : ego_id_(id), agent_config_path_(agent_config_path), is_auto_forward_(is_auto_forward) {
  p_config_loader_ = new ConfigLoader();
  p_config_loader_->set_ego_id(ego_id_);
  p_config_loader_->set_agent_config_path(agent_config_path_);
  p_config_loader_->ParseAgentConfig(&agent_config_info_);
  ...
  if (is_auto_forward_) {
    forward_kl_planner_.set_user_desired_velocity(agent_config_info_.desire_velocity);
    forward_kl_planner_.set_reference_desired_velocity(agent_config_info_.desire_velocity);
  }
}

EgoSemanticMapManager::EgoSemanticMapManager(
...
// ErrorType EgoSemanticMapManager::UpdateSemanticMap(
...
//   ego_behavior_.ref_lane = local_lanes_.at(lane_id);
//   return kSuccess;
// }
...
ErrorType EgoSemanticMapManager::UpdateSemanticMap(
    ...
    const common::SemanticBehavior &behavior,
    const std::string &behavior_uid) {
  while (true) {
    if (semantic_map_updated_) break;
    // 必須等待語義地圖被第一次更新後，才能執行下面的
    // NOTE for MLAD: 这里会是多线程的，必须要等另外一个Update方法更新后，这里才能进行
  }
  TicToc timer;
  // time_stamp_ = time_stamp;
  set_ego_behavior(behavior);
  set_cloud_behavior_uid(behavior_uid);
  return kSuccess;
}
...
ErrorType EgoSemanticMapManager::UpdateSemanticMap(
  ...
  TicToc timer;
  time_stamp_ = time_stamp;
  // printf("[EgoSemanticMapManager::UpdateSemanticMap] Update from phy_simulator \n");
  set_ego_vehicle(ego_vehicle);
  set_whole_lane_net(whole_lane_net);
  set_surrounding_lane_net(surrounding_lane_net);
  set_obstacle_map(obstacle_map);
  ...
  semantic_map_updated_ = true;
  return kSuccess;
}

ErrorType EgoSemanticMapManager::SaveMapToLog() {
...
ErrorType EgoSemanticMapManager::NaiveRuleBasedLateralBehaviorPrediction(
...
ErrorType EgoSemanticMapManager::MobilRuleBasedBehaviorPrediction(
...
ErrorType EgoSemanticMapManager::GetLeadingAndFollowingVehiclesFrenetStateOnLane(
...
ErrorType EgoSemanticMapManager::GetEgoNearestLaneId(int *ego_lane_id) const {
  int nearest_lane_id;
...
ErrorType EgoSemanticMapManager::UpdateSemanticVehicles() {
...
ErrorType EgoSemanticMapManager::OpenloopTrajectoryPrediction() {
...
ErrorType EgoSemanticMapManager::UpdateSemanticLaneSet() {
...
ErrorType EgoSemanticMapManager::UpdateLocalLanesAndFastLut() {
  ...
  std::unordered_map<int, common::Lane> local_lanes_temp;
  std::unordered_map<int, std::vector<int>> local_to_segment_lut_temp;
  std::unordered_map<int, std::set<int>> segment_to_local_lut_temp;
  ...
  std::vector<int> root_lane_ids;
  {
    ...
    if (whole_lane_net_.lane_set.at(cur_lane_id).l_lane_id > 0 
        && surrounding_lane_net_.lane_set.find(whole_lane_net_.lane_set.at(cur_lane_id).l_lane_id) 
        != surrounding_lane_net_.lane_set.end()) {
      ...
      if (whole_lane_net_.lane_set.at(l_id).l_lane_id > 0
          && surrounding_lane_net_.lane_set.find(whole_lane_net_.lane_set.at(l_id).l_lane_id) 
          != surrounding_lane_net_.lane_set.end()) {
    ...
    if (whole_lane_net_.lane_set.at(cur_lane_id).r_lane_id > 0
        && surrounding_lane_net_.lane_set.find(whole_lane_net_.lane_set.at(cur_lane_id).r_lane_id) 
        != surrounding_lane_net_.lane_set.end()) {
      ...
      if (whole_lane_net_.lane_set.at(r_id).r_lane_id > 0
          && surrounding_lane_net_.lane_set.find(whole_lane_net_.lane_set.at(r_id).r_lane_id) 
          != surrounding_lane_net_.lane_set.end()) {
  ...
  for (const auto root_id : root_lane_ids) {
    ...
    int local_lane_cnt = local_lanes_temp.size();
    for (const auto &path : assembled_paths) {
      common::Lane lane;
      if (kSuccess !=
      ...
      local_lanes_temp.insert(std::pair<int, common::Lane>(local_id, lane));

      local_to_segment_lut_temp.insert(
          std::pair<int, std::vector<int>>(local_id, path));

      for (const auto &id : path) {
        segment_to_local_lut_temp[id].insert(local_id);
      }
    }
  }
  ...
  //   printf("\n");
  // }
  std::swap(local_lanes_, local_lanes_temp);
  std::swap(local_to_segment_lut_, local_to_segment_lut_temp);
  std::swap(segment_to_local_lut_, segment_to_local_lut_temp);
  has_fast_lut_ = true;
  return kSuccess;
}

void EgoSemanticMapManager::GetAllForwardLaneIdPathsWithMinimumLengthByRecursion(
...
void EgoSemanticMapManager::GetAllBackwardLaneIdPathsWithMinimumLengthByRecursion(
    const decimal_t &node_id, const decimal_t &node_length,
    const decimal_t &aggre_length, const std::vector<int> &path_to_node,
    std::vector<std::vector<int>> *all_paths) {
  // * Traverse BACKWARD
...
ErrorType EgoSemanticMapManager::GetDistanceToLanesUsing3DofState(
    const Vec3f &state,
...
ErrorType EgoSemanticMapManager::CheckCollisionUsingState(
...
ErrorType EgoSemanticMapManager::CheckCollisionUsingStateAndVehicleParam(
...
ErrorType EgoSemanticMapManager::CheckCollisionUsingGlobalPosition(
...
ErrorType EgoSemanticMapManager::GetObstacleMapValueUsingGlobalPosition(
    const Vec2f &p_w, ObstacleMapType *res) {
  std::array<decimal_t, 2> p = {{p_w(0), p_w(1)}};
  return obstacle_map_.GetValueUsingGlobalPosition(p, res);
}

ErrorType EgoSemanticMapManager::IsTopologicallyReachable(
...
ErrorType EgoSemanticMapManager::GetNearestLaneIdUsingState(
...
ErrorType EgoSemanticMapManager::TrajectoryPredictionForVehicle(
...
ErrorType EgoSemanticMapManager::UpdateKeyVehicles() {
...
ErrorType EgoSemanticMapManager::GetLocalLaneSamplesByState(
...
ErrorType EgoSemanticMapManager::GetLocalLaneUsingLaneIds(
...
ErrorType EgoSemanticMapManager::GetRefLaneForStateByBehavior(
...
ErrorType EgoSemanticMapManager::GetRefLaneForStateByBehavior(
    ...
    const LateralBehavior &behavior, const decimal_t &max_forward_len,
    const decimal_t &max_back_len, const bool is_high_quality,
    common::Lane *lane, int *lane_id) const {
  Vec3f state_3dof(state.vec_position(0), state.vec_position(1), state.angle);
  int current_lane_id;
  decimal_t distance_to_lane;
  decimal_t arc_len;
...
ErrorType EgoSemanticMapManager::GetLaneBySampledPoints(
...
ErrorType EgoSemanticMapManager::SampleLane(const common::Lane &lane,
                                         const decimal_t &s0,
                                         const decimal_t &s1,
...
ErrorType EgoSemanticMapManager::GetTargetLaneId(const int lane_id,
...
ErrorType EgoSemanticMapManager::GetLeadingVehicleOnLane(
    const common::Lane &ref_lane, const common::State &ref_state,
...
ErrorType EgoSemanticMapManager::GetFollowingVehicleOnLane(
    const common::Lane &ref_lane, const common::State &ref_state,
    const common::VehicleSet &vehicle_set, const decimal_t &lat_range,
  ...
  for (decimal_t s = ref_fs.vec_s[0] - resolution - search_lon_offset;
       s > ref_fs.vec_s[0] - max_forward_search_dist - search_lon_offset;
       s -= resolution) {
    if (ref_lane.GetPositionByArcLength(s, &lane_pt) != kSuccess) {
      continue;
    }
    // lane_pt = lane_pt + offset;
    for (const auto &entry : vehicle_set.vehicles) {
...
ErrorType EgoSemanticMapManager::GetSpeedLimit(const State &state,
                                            const Lane &lane,
                                            decimal_t *speed_limit) const {
  return traffic_singal_manager_.GetSpeedLimit(state, lane, speed_limit);
}

ErrorType EgoSemanticMapManager::GetTrafficStoppingState(
...
bool EgoSemanticMapManager::IsLocalLaneContainsLane(const int &local_lane_id,
...
ErrorType EgoSemanticMapManager::GetDistanceOnLaneNet(const int &lane_id_0,
...
ErrorType EgoSemanticMapManager::UpdateEgoSemanticMap() {
  TicToc timer;
  ...
  // * update semantic info for vehicles
  UpdateSemanticVehicles(); // TODO for MLAD: 合作agv不需要预测轨迹，只有非合作agv才要预测轨迹。如果合作agv未上传轨迹，怎么处理？或者上传轨迹早于查看轨迹的时间点，怎么处理
...
bool EgoSemanticMapManager::ReplanEgoBehavior(const common::SemanticBehavior &behavior) {
  if (behavior.forward_behaviors.empty()) {
    return true;
  }

  if (behavior.forward_trajs.empty()) {
    return true;
  }

  if (behavior.forward_trajs.front().empty()) {
    return true;
  }

  decimal_t keep_lane_plan_t = time_stamp_ + agent_config_info_.advanced_time_for_keep_lane_plan;
  if (behavior.forward_trajs.front().back().state().time_stamp < keep_lane_plan_t) {
    return true;
  }
...
common::SemanticBehavior EgoSemanticMapManager::ego_behavior() {
  if (!is_auto_forward_) return ego_behavior_;
  
  common::SemanticBehavior cur_behavior = ego_behavior_;

  if (!semantic_map_updated_) 
  {
    printf("[EgoSemanticMapManager::ego_behavior] The semanticMap is not update by environment.\n");
    return cur_behavior;
  }
  // 必须等semantic map第一次更新后，才能执行下面的
  

  // Get ref_lane
  Vec3f state_3dof(ego_vehicle_.state().vec_position(0), ego_vehicle_.state().vec_position(1), ego_vehicle_.state().angle);
  int current_lane_id;
  decimal_t distance_to_lane;
  decimal_t arc_len;
  if (GetNearestLaneIdUsingState(state_3dof, std::vector<int>(), &current_lane_id,
                                &distance_to_lane, &arc_len) != kSuccess) {
    printf("[EgoSemanticMapManager::ego_behavior]Cannot get nearest lane.\n");
    return cur_behavior;
  }
  ...
  if (!agent_config_info_.enable_fast_lane_lut || !has_fast_lut_) {
    printf("[EgoSemanticMapManager::ego_behavior] Cannot get ref lane.\n");
    return cur_behavior;
  }

  if (segment_to_local_lut_.end() == segment_to_local_lut_.find(current_lane_id)) {
    printf("[EgoSemanticMapManager::ego_behavior] Cannot get ref lane.\n");
    return cur_behavior;
  }
  // * here we just select the first local lane from several candidates
  int id = *segment_to_local_lut_.at(current_lane_id).begin();
  cur_behavior.ref_lane = local_lanes_.at(id);
  ...
  if (ReplanEgoBehavior(cur_behavior)) {
    ...
    // {
    //   std::lock_guard<std::mutex> lock_guard(behavior_update_mtx_); 
    //   // 互斥锁，避免此时云端更新behavior
    //   if (ReplanEgoBehavior(ego_behavior_)) {
    //     // 若在执行上面的时候，云端没有更新，则将新的behavior进行更新
    //     ego_behavior_ = cur_behavior;
    //   }
    // }
  }
  else {
    // TODO for MLAD: 在这里，需要对behavior中的轨迹，进行过滤，将其中早于当前时间的state，去掉
    UpdateBehaviorWithCurrentTime(&cur_behavior);
  }
  
  return cur_behavior;
}

ErrorType EgoSemanticMapManager::UpdateBehaviorWithCurrentTime(common::SemanticBehavior *behavior) {
  for (auto straj = behavior->surround_trajs.begin(); straj != behavior->surround_trajs.end(); straj++) {
    for (auto &[sid, traj]:(*straj)) {
      std::reverse(traj.begin(), traj.end());
      while (!traj.empty()) {
        if (traj.back().state().time_stamp < time_stamp_) {
          traj.pop_back();
        }
        else {
          break;
  ...
  bool empty_behavior = true;
  for (auto traj = behavior->forward_trajs.begin(); traj != behavior->forward_trajs.end(); traj++) {
    std::reverse(traj->begin(), traj->end());
    while (!traj->empty()) {
      if (traj->back().state().time_stamp < time_stamp_) {
        traj->pop_back();
    ...
    std::reverse(traj->begin(), traj->end());
    empty_behavior &= traj->empty();
  }

  if (empty_behavior) {
    behavior->forward_behaviors.clear();
    behavior->surround_trajs.clear();
    behavior->forward_behaviors.clear();
  }
  return kSuccess;
}

ErrorType EgoSemanticMapManager::priority_order_score(const int &id, std::pair<int, decimal_t> *order_score) const {
  if (priority_scores_.find(id) == priority_scores_.end()) {
    return kWrongStatus;
  }

  *order_score = priority_scores_.at(id);
