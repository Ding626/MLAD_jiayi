...
EgoDataRenderer::EgoDataRenderer(EgoSemanticMapManager *smm_ptr)
  ...
  printf("[EgoDataRenderer] Initialization finished\n");
}

ErrorType EgoDataRenderer::Render(const double &time_stamp,
  ...
  p_semantic_map_manager_->UpdateSemanticMap(
      time_stamp_, behavior, behavior_uid);
  return kSuccess;
}
...
ErrorType EgoDataRenderer::Render(const double &time_stamp,
  ...
  decimal_t lane_dir{0.0};
  for (const auto &l:lane_net.lane_set) {
    if (l.second.is_multi_lane_route) {
      lane_dir = l.second.dir;
      break;
    }
  }
  if (!SemanticMapManager::IsSameOrientation(ego_state_.angle, lane_dir)) {
    common::LaneNet new_lane_net = lane_net;
    GetWholeLaneNet(new_lane_net.reverse());
    GetSurroundingLaneNet(new_lane_net.reverse());
  }
  else {
  ...
  p_semantic_map_manager_->UpdateSemanticMap(
      ...
      *p_obstacle_grid_, obs_grids_, surrounding_vehicles_, obstacle_set);

  return kSuccess;
}

ErrorType EgoDataRenderer::InjectObservationNoise() {
...
ErrorType EgoDataRenderer::GetEgoVehicle(const common::VehicleSet &vehicle_set) {
...
ErrorType EgoDataRenderer::GetObstacleMap(
    const common::ObstacleSet &obstacle_set) {
  // ~ NOTICE:
...
ErrorType EgoDataRenderer::FakeMapper() {
...
ErrorType EgoDataRenderer::RayCastingOnObstacleMap() {
...
ErrorType EgoDataRenderer::GetWholeLaneNet(const common::LaneNet &lane_net) {
  whole_lane_net_ = lane_net;
  return kSuccess;
}

ErrorType EgoDataRenderer::GetSurroundingLaneNet(const common::LaneNet &lane_net) {
...
ErrorType EgoDataRenderer::GetSurroundingVehicles(
    const common::VehicleSet &vehicle_set) {
