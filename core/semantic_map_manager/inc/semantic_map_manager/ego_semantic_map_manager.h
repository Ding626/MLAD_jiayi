#ifndef _CORE_SEMANTIC_MAP_INC_EGO_SEMANTIC_MAP_MANAGER_SEMANTIC_MAP_H_
#define _CORE_SEMANTIC_MAP_INC_EGO_SEMANTIC_MAP_MANAGER_SEMANTIC_MAP_H_
...
class EgoSemanticMapManager {
 public:
  ...
  EgoSemanticMapManager() {}
  EgoSemanticMapManager(const int &id, const std::string &agent_config_path);
  EgoSemanticMapManager(const int &id, const std::string &agent_config_path,
                        const bool &is_auto_forward);
  EgoSemanticMapManager(const int &id, const decimal_t surrounding_search_radius,
  ...
  common::SemanticBehavior ego_behavior();

  ErrorType priority_order_score(const int &id, std::pair<int, decimal_t> *order_score) const;
  ...
  inline std::string cloud_behavior_uid() {
    return cloud_behavior_uid_;
  }
  ...
  inline void set_agent_config_info(const AgentConfigInfo &in) {
    agent_config_info_ = in;
  }

  inline void set_cloud_behavior_uid(const std::string &uid) {
    cloud_behavior_uid_ = uid;
  }

  inline void set_priority_scores(const std::unordered_map<int, std::pair<int, decimal_t>> &p_scores) {
    priority_scores_.clear();
    priority_scores_ = p_scores;
  }

  inline void set_time_stamp(const double &in) { time_stamp_ = in; }

  ErrorType UpdateEgoSemanticMap(); // 用于云端，生成行为规划时所需要的EgoSemanticMapManager
 ...
 private:
  ...
  ErrorType UpdateBehaviorWithCurrentTime(common::SemanticBehavior *behavior);
  ...
  bool ReplanEgoBehavior(const common::SemanticBehavior &behavior);
  ...
  bool semantic_map_updated_ = false; // 语义地图是否初次被更新过

  // * For highway-like lane structure only
  bool is_simple_lane_structure_ = false;

  bool is_auto_forward_ = false;

  ForwardPlanner forward_kl_planner_; // 直行且保持当前车道的planner

  std::string cloud_behavior_uid_; // 云端下发的最新的behavior的uid
  // std::mutex cloud_behavior_uid_mtx_;
  // NOTE for MLAD: 由于EgoSemanticMapManager调用，都是通过生产者-消费者队列来解决多线程调用问题的，在ssc中，调用smm时，smm不会被别的线程改变了，且每次ssc运行一次，都是一个新的smm，所以不需要在这里加锁
  // std::mutex behavior_update_mtx_; // 用于云端下发behavior，与直行生成behavior更新的互斥锁
};
