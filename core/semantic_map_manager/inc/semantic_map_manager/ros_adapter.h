...
class RosAdapter {
 public:
  ...
  RosAdapter(ros::NodeHandle nh, SemanticMapManager* ptr_smm) : nh_(nh) {
    ...
    _vehicle_traj_sub_set.resize(p_smm_->max_ego_vehicle_num());
  }
  ~RosAdapter() {}
 ...
 private:
  ...
  void VehicleTrajectoryCallback(
      const vehicle_msgs::TrajUpdateInfo::ConstPtr& msg);

  void VehicleTaskCallback(const vehicle_msgs::TaskSet::ConstPtr& msg);
  ros::NodeHandle nh_;
  ...
  // ros::Subscriber vehicle_response_sub_;
  std::vector<ros::Subscriber> _vehicle_traj_sub_set;

  common::VehicleSet vehicle_set_;
  common::LaneNet lane_net_;
  common::ObstacleSet obstacle_set_;
  common::TaskSet task_set_;
