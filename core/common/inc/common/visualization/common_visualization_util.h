...
class VisualizationUtil {
 public:
  ...
  static ErrorType GetRosMarkerArrayUsingVehicle(
    ...
    if (vehicle.ready_change_lane()) {
      p_marker_array->markers.push_back(obb_marker);
      // p_marker_array->markers.push_back(mesh_marker); // NOTE for MLAD: 所有key車都顯示爲車的圖標
    } else {
      p_marker_array->markers.push_back(obb_marker);
    }
   ...
   * @return ErrorType
   */
  static ErrorType GetRosMarkerArrUsingLaneWithDirection(
      const Lane& ref_lane,
      const bool &cur_pass_dir_straight,
      visualization_msgs::MarkerArray* p_marker_array) {
    ...
    if (ref_lane.IsValid()) {
      ...
      FillScaleColorInMarker(Vec3f(0.2, 0.0, 0.0),
                              ColorARGB(1.0, 0.1, 0.8, 0.1), &direction_mk);
      angle = kPi / 4.0;
    
      const decimal_t sample_step = 5.0;
      const decimal_t arrow_width = 0.5;
      for (decimal_t s = ref_lane.begin(); s < ref_lane.end(); s += sample_step) {
        Vecf<2> pos;
        ref_lane.GetPositionByArcLength(s, &pos);
        Vecf<2> normal_vec;
        ref_lane.GetNormalVectorByArcLength(s, &normal_vec);
        ...
        ConvertVectorToPoint<2>(pos, &origin);
        {
          ...
          Vecf<2> left = pos + arrow_width / acos(angle) *
                                    rotate_vector_2d(normal_vec, cur_pass_dir_straight ? angle : -angle);
