...
class Vehicle {
 public:
  ...
  Vehicle(const int &id, const VehicleParam &param, const State &state, const bool &ready_change_lane);
  Vehicle(const int &id, const std::string &subclass, const VehicleParam &param,
          const State &state);
  Vehicle(const int &id, const std::string &subclass, const VehicleParam &param,
          const State &state, std::string &name, const bool &ready_change_lane);
  Vehicle(const int &id, const std::string &subclass, const VehicleParam &param, 
          const State &state, std::string &name, const bool &ready_change_lane, 
  ...
  inline bool ready_change_lane() const { return ready_change_lane_; }
  ...
  inline void set_ready_change_lane(const bool &ready_change_lane) { 
    ready_change_lane_ = ready_change_lane; 
  }
 ...
 private:
  ...
  State state_;
  std::string name_;
  bool ready_change_lane_;
  // TODO for MLAD: 需要在这里加入一个属性，表示其对应的车道id，以及是否需要对其进行变道决策、行为规划等（即是否进入缓冲区）
};
...
struct LaneRaw {
  ...
  std::string behavior; // "s"表示多车道路径中的车道，支持变道，"n"表示普通的路径，不支持变道，但能行走，"p"表示路径不能行走
  decimal_t length;
  ...
  decimal_t dir; // 车道主方向
  decimal_t buffer; // 缓冲区的长度
  int group; // 多径通道的组号
  bool is_multi_lane_route; // 是否是多径通道
  bool dir_changeable; //此车道的通行方向是否是可变的，主要是针对多径车道
  /**
   * @brief Print info
   */
  void print() const;
  ...
  void reverse() {
    // TODO for MLAD: 暫時還不涉及child id和father id，後續可能要改
    std::swap(start_point, final_point);
    std::reverse(lane_points.begin(), lane_points.end());
    std::swap(child_id, father_id);
  }
};
...
struct LaneNet {
  ...
  LaneNet reverse() {
    LaneNet new_lane_net;
    for (auto iter = lane_set.begin(); iter != lane_set.end(); iter++) {
      LaneRaw new_lane = iter->second;
      new_lane.reverse();
      new_lane_net.lane_set.insert(std::make_pair(iter->first, new_lane));
    }
    return new_lane_net;
  }
};
