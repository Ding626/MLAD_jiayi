...
// 模拟HDV的程序，只考虑前方的一辆车辆（在车道左右周边2.2米范围内的），使用IDM直接求出此时的control signal
// desire速度会随机增加或减小


ros::Publisher ctrl_signal_pub_;
planning::BehaviorPlannerServer* p_bp_server_{nullptr};
moodycamel::ReaderWriterQueue<semantic_map_manager::EgoSemanticMapManager>*
...
semantic_map_manager::EgoSemanticMapManager last_smm;
...
int SemanticMapUpdateCallback(
    const semantic_map_manager::EgoSemanticMapManager& smm) {
...
void RandomBehavior() {
  if (cnt == 0) {
    std::uniform_real_distribution<double> dist_vel(random_lb, random_ub);
...
int main(int argc, char** argv) {
  ...
  nh.param("random_lb", random_lb, -2);
  nh.param("random_ub", random_ub, 5);
  ...
  // Declare smm
  semantic_map_manager::EgoSemanticMapManager semantic_map_manager(
      ego_id, agent_config_path);
  semantic_map_manager::EgoRosAdapter smm_ros_adapter(nh, &semantic_map_manager);
  ...
  p_ctrl_input_smm_buff_ = new moodycamel::ReaderWriterQueue<
      semantic_map_manager::EgoSemanticMapManager>(100);
