...
class EudmPlannerMapItf {
 public:
  ...
  virtual ErrorType GetBehavioralKeyVehiclesIds(
      std::unordered_set<int> *behavioral_key_vehicle_ids) = 0;
  virtual ErrorType IsBehavioralVehicleRunningPlannedTraj(const int &behavioral_key_vehicle_id, 
      const decimal_t &time, bool *is_running) = 0;
  virtual ErrorType GetRunningBehavioralVehicleStateAtSpecTime(const int &behavioral_key_vehicle_id, 
      const decimal_t &time, common::State *state_output) = 0;
	virtual ErrorType IsEgoPriorityHigher(const int &ego_vehicle_id, const int &compare_id, 
			bool *is_ego_prior) = 0;
};
