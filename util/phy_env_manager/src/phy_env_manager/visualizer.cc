...
void Visualizer::VisualizeDataWithStamp(const ros::Time &stamp) {
  VisualizeVehicleSet(stamp, p_phy_manager_->vehicle_set());
  VisualizeLaneNet(stamp, p_phy_manager_->lane_net());
  VisualizeObstacleSet(stamp, p_phy_manager_->obstacle_set());
}

// void Visualizer::SendTfWithStamp(const ros::Time &stamp) {
//   auto vehicle_set = p_phy_manager_->vehicle_set();
